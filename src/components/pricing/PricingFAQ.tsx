      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          <div className="md:hidden w-full flex justify-center mb-4">
            <img src={`${IMG}/pet-grooming-prices/faqs-mobile.png`} alt="FAQs" className="w-full max-w-[280px] object-contain" />
          </div>

          {/* FAQ accordion */}
          <div className="flex-1">
            <h2 className="ff-inter-semibold text-[#2a2a2a] text-2xl md:text-[1.75rem] mb-6">FAQ</h2>
            {faqs.map((faq, idx) => (
              <div key={idx} className="border-b border-[#e5e7eb]">
                <button
                  onClick={() => setOpenIdx(openIdx === idx ? -1 : idx)}
                  className="w-full flex items-center justify-between py-5 bg-transparent border-0 cursor-pointer text-left"
                >
                  <span className="ff-inter-medium text-[#2a2a2a] text-sm md:text-base pr-4">{faq.q}</span>
                  <span className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-colors ${openIdx === idx ? "bg-[#2a2a2a]" : "bg-[#f0f0f0] border border-[#e5e7eb]"}`}>
